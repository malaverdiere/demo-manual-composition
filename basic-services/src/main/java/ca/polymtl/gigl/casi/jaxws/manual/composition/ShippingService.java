...
@WebService(targetNamespace = "http://ca.polymtl.gigl.casi.jaxws-manual-composition", name = "ShippingService")
public interface ShippingService {
    @WebMethod(operationName = "estimate")
    public BigDecimal calculateEstimate(
            @WebParam(name = "address") Address destination,
            @WebParam(name = "weight")  float weight,
            @WebParam(name = "size")    float size);
